...
import type { PrevMonthTail } from "../../../lib/schedules/monthly-schedule-engine";
...
describe("cross-month — applyCrossMonthWeekendPack", () => {
  it("does nothing for empty prevMonthTail", () => {
    ...
    const stateMap = new Map<string, EmployeeState>();
  ...
  it("pre-seeds weekend plan when Jan ends Saturday and Feb starts Sunday", () => {
    ...
    const weekendPlan = new Map<string, WeekendPackage>();
    const e1State: EmployeeState = { weekendShift: new Map<string, string>(), weekShift: new Map<string, string>() };
    const e2State: EmployeeState = { weekendShift: new Map<string, string>(), weekShift: new Map<string, string>() };
